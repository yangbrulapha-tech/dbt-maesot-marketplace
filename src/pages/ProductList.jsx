...
export default function ProductList({ session }) {
  ...
  const handleDeleteProduct = async () => {
    ...
    const productTitle = productToDelete.title || 'สินค้า'
    const productSellerId = String(productToDelete.student_id || productToDelete.seller_id || '').trim()
    const currentStudentId = String(userProfile.student_id || '').trim()
    ...
    if (!isSeller && !isAdmin) {
      addToast('คุณไม่มีสิทธิ์ลบสินค้าชิ้นนี้', 'error')
      return
    }

    setDeleteLoadingId(productId)
    try {
      // 1. ลองลบแถวสินค้าออกจากฐานข้อมูลโดยตรง
      let { error } = await supabase.from('products').delete().eq('product_id', productId)

      // 2. หากติด Constraint จากประวัติออเดอร์เดิม ให้อัปเดตสถานะเป็น 'deleted' หรือ 'sold_out' เพื่อซ่อนจากหน้าตลาดทันที
      if (error) {
        const fallbackRes = await supabase.from('products').update({ status: 'deleted', stock: 0 }).eq('product_id', productId)
      ...
      addToast('ไม่สามารถลบสินค้าได้: ' + (err.message || 'เกิดข้อผิดพลาดในการลบ'), 'error')
    } finally {
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="mt-2.5 flex items-center justify-between">
                      ...
                      <span className={`text-xs font-black px-2.5 py-1 rounded-full border shadow-sm flex items-center space-x-1 ${
                  ...
                  {isSeller ? (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button onClick={() => openMessage(product)}
                        className="flex items-center justify-center space-x-1 border border-slate-300 hover:border-navy-600 hover:text-navy-900 dark:text-white bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold py-2 rounded-lg text-xs transition-all">
                        <MessageSquare className="h-3.5 w-3.5" /><span>ส่งข้อความ</span>
                      </button>
                      <button onClick={() => openCheckout(product)}
                        className="flex items-center justify-center space-x-1 bg-primary-600 hover:bg-primary-500 text-white font-bold py-2 rounded-lg text-xs shadow-sm transition-all">
